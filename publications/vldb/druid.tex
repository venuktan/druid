...
\begin{abstract}
Druid is an open source\footnote{https://github.com/metamx/druid}, real-time analytical data store that supports
...
core engine of the Metamarkets interactive data analysis platform. Here, we detail Druid's architecture, and describe how it supports real-time data ingestion, querying, and analytics. 
\end{abstract}
...
is partitioned into a collection of segments, each typically comprising 5--10 million rows.
...
Most segments in a Druid cluster will be immutable \emph{historical} segments. Such segments are persisted on local disk or in a distributed filesystem such as S3 \cite{decandia2007dynamo} or HDFS \cite{shvachko2010hadoop}. All historical
...
Segments covering very recent intervals will be mutable \emph{real-time} segments. Real-time segments are incrementally updated as new events are ingested. Periodically, real-time segments are converted into
historical segments through a finalization and handoff process described in Section~\ref{sec:realtime}.

Druid is best used for aggregating event streams, and both historical and real-time segments are built through an incremental indexing process that takes advantage of this assumption. Incremental indexing works by computing running aggregates of interesting metrics (number of impressions, sum of revenue, etc) across all rows that have identical attributes (publisher, advertiser, etc). This often produces an order of magnitude compression in the data without sacrificing analytical value. Of course, indexing raw data is an option as well.
